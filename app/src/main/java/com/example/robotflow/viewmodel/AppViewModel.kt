...
class AppViewModel(application: Application) : AndroidViewModel(application) {
    ...
    private fun sendEsp32Command(command: String) {
        ...
        Log.d("ESP32_COMMAND", "Raw Tx: $formattedCommand")

        if (usbSerialPort == null || usbSerialPort?.isOpen == false) return

        viewModelScope.launch(Dispatchers.IO) {
            try {
                usbSerialPort?.write(formattedCommand.toByteArray(), 500)
            } catch (e: Exception) {
                withContext(Dispatchers.Main) { usbConnectionState = "Tx Failed" }
            }
        }
    }
